...
class ListPost extends StatefulWidget {
  ...
  const ListPost({Key? key, this.scrollController}) : super(key: key);
...
class _ListPostState extends State<ListPost> {
  ...
  Widget build(BuildContext context) {
    ...
    //     value: _userService
    //         .findUsersWithEpisodes(FirebaseAuth.instance.currentUser!.uid),
    //     initialData: const <MangoUser>[],
    //     child: const StoryCircle(),
    //   ),
    // ),
...
class _PostItemState extends State<PostItem> {
  ...
  // String myName = '_topboy_hq';
  //
  // ///List of all the names in the group - Assuming here that you get this list in chat window
  // List<String> otherNamesList = ['_gun', '_topboy_hq', 'Dwight Schrute', "Jim Halpert"];
  ...
  final MangoPost _post = MangoPost(link: '', replyTo: '', linkType: '', isLinked: false, reply: false, repost: false, likeCount: 0, replyCount: 0, repostCount: 0, text: '', postCreator: '', image: '', timeStamp: null, id: '', multiImage: [], multiMedia: false, video: '');
  @override
  Widget build(BuildContext context) {
    ...
    try {
        ...
        // Uri uri = Uri.parse();
        file = await toFile(widget.post.video!);
        setState((){
          video = file!;
        });
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const ShimmerWidget.circular(width: 36, height: 36),
                const SizedBox(
                  width: 10,
                ),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: const [
                    ShimmerWidget.rectangularCircular(width: 100, height: 14),
            ...
            const ShimmerWidget.rectangularCircular(
                        ...
                        children: [
                                ...
                                onTap: () => Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) => ProfilePage(
                                            userId: widget.snapshot.data!.id))),
                                child: Row(
                                  children: [
                                    CachedNetworkImage(
                                      height: 38,
                                      imageUrl: widget.snapshot.data!.profileImage,
                          ...
                          FutureBuilder<List>(
                              ...
                              return MarkdownBody(
                                ...
                                builders: {
                                  "coloredBox": ColoredBoxMarkdownElementBuilder(context, usernamesSnapshot.data!, thisUser.data!.username),
                                },
                                inlineSyntaxes: [
